...
def create_mask_from_polygon_data(data, root_folderpath: pathlib.Path):
    width = data["tag"]["original_width"]
    height = data["tag"]["original_height"]
    mask = np.zeros((height, width), dtype=np.uint8)
    brushlabel = data["tag"]["brushlabels"]
    rle_str = data["tag"]["rle"]
    
    print(rle_str)
    rle_mask = rle2mask(rle_str, height, width)
    print(rle_mask)



    #TODO This needs to be the points (rle format) that are already unscaled, but need to only use the prosome brushlabels instead of polygonlabels
    # rle_mask = rle_decode(rle_str = data["tag"]["rle"], shape = np.stack([1,1]))
    ...
    output_path = (
        root_folderpath / f"{data['image'][-40:-4]}-{data['tag']['polygonlabels'][0]}_mask.png"
    )
...
def create_mask_from_csv(csv_path, masks_dst_folderpath, num_threads=4):
    ...
    # Apply the necessary transformations to the "tag" column
    df["tag"] = df["tag"].apply(lambda x: x.replace("true", "True"))
...
if __name__ == "__main__":
    csv_path = r"C:\Laval_Postdoc\Laval-imaging-analysis\mask_vs_points\project-12-masks.csv"
    root_folderpath = r"C:\Laval_Postdoc\Laval-imaging-analysis\mask_vs_points\masks\project12"
